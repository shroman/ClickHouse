...
bool ReplicatedMergeTreeQueue::addFuturePartIfNotCoveredByThem(const String & part_name, const LogEntryPtr & entry, String & reject_reason)
{
    std::lock_guard<std::mutex> lock(mutex);
...
void ReplicatedMergeTreeQueue::CurrentlyExecuting::setActualPartName(const ReplicatedMergeTreeQueue::LogEntryPtr & entry,
                                                                     const String & actual_part_name, ReplicatedMergeTreeQueue & queue)
{
    ...
    if (!queue.future_parts.emplace(entry->actual_new_part_name, entry).second)
