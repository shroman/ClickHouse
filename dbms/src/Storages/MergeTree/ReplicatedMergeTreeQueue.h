...
private:
    ...
    private:
        ...
        static void setActualPartName(const ReplicatedMergeTreeQueue::LogEntryPtr & entry, const String & actual_part_name,
            ReplicatedMergeTreeQueue & queue);
    public:
        ~CurrentlyExecuting();
    };
...
public:
    ...
    /** Check that part isn't in currently generating parts and isn't covered by them and add it to future_parts.
      * Locks queue's mutex.
      */
    bool addFuturePartIfNotCoveredByThem(const String & part_name, const LogEntryPtr & entry, String & reject_reason);
